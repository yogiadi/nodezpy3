...
def on_nodeEdited(nodeName, newName):
    print ('node edited :' +  nodeName+' new name :'+ newName)

@QtCore.Slot(str)
def on_nodeSelected(nodesName):
    print ('node selected : '+ str(nodesName))

@QtCore.Slot(str, object)
def on_nodeMoved(nodeName, nodePos):
    print ('node ' + nodeName+ 'moved to '+ str(nodePos))

@QtCore.Slot(str)
def on_nodeDoubleClick(nodeName):
    print ('double click on node :'+nodeName)

# Attrs
@QtCore.Slot(str, int)
def on_attrCreated(nodeName, attrId):
    print ('attr created : ' + nodeName + 'at index :'+ str(attrId))

@QtCore.Slot(str, int)
def on_attrDeleted(nodeName, attrId):
    print ('attr Deleted :'+ nodeName +' at old index :'+ str(attrId))

@QtCore.Slot(str, int, int)
def on_attrEdited(nodeName, oldId, newId):
    print ('attr Edited : ' + nodeName+' at old index : '+str(oldId)+', new index : {2}'+ str(newId))

# Connections
@QtCore.Slot(str, str, str, str)
def on_connected(srcNodeName, srcPlugName, destNodeName, dstSocketName):
    print ('connected src: "{0}" at "{1}" to dst: "{2}" at "{3}"')\
        # .format(srcNodeName, srcPlugName, destNodeName, dstSocketName)

@QtCore.Slot(str, str, str, str)
def on_disconnected(srcNodeName, srcPlugName, destNodeName, dstSocketName):
    print ('disconnected src: "{0}" at "{1}" from dst: "{2}" at "{3}"')\
        # .format(srcNodeName, srcPlugName, destNodeName, dstSocketName)
